...
/**
 ...
 Property 1: "when $var is int and $type is 'numeric' the result is always 'true'"  
	...
	- input    = 'ListItem(#symInt(x)) ListItem("numeric") ListItem(#symInt(x))' 
	- formula  = '<>Ltl (eqTo(gv(variable("result")),val(true)))'

  Property 2: "PMA_is_Valid($x, 'similar' $y) always return true when $x is int and $y is string"

  	- input    = 'ListItem(#symInt(x)) ListItem("similar") ListItem(#symString(x))' 
  	-formula   = '<>Ltl(eqTo(gv(variable("result")),val(true)))'	
 **/
